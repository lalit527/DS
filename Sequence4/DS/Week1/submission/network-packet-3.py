...
class Buffer:
    ...
    def Process(self, request):
        ...
        _sum = request.process_time
        while len(self.finish_time_) > 0:
        ...
        if len(self.finish_time_) >= self.size:
          ...
          if len(self.finish_time_) == 0:
            ...
            last = self.finish_time_[- 1]
            if request.arrival_time >= last:
              self.finish_time_.append(request.arrival_time + request.process_time)
              return Response(False, request.arrival_time)
            else:
              self.finish_time_.append(last + request.process_time)
